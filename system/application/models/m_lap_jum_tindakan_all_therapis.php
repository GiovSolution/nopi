<? /* 
	+ Module  		: Laporan Tindakan Semua Therapis Model
	+ Description	: For record model process back-end
	+ Filename 		: c_lap_jum_tindakan_all_therapis.php
 	+ Author  		: Isaac

*/

class m_lap_jum_tindakan_all_therapis extends Model{
		
	//constructor
	function m_lap_jum_tindakan_all_therapis() {
		parent::Model();
	}		
	...
	function report_tindakan_update_temp($isiperiode, $tglpaket, $dokter_id, $report_groupby, $urutan, $cabang){
		...
		foreach($result_query->result() as $row_query){
			$sql_update =  "update temp_jml_tindakan_therapis t
							set t.tjtt_ref".$urutan." = ".$row_query->Jumlah_rawat."
							where t.tjtt_rawat = ".$row_query->perawatan_id;
	...
	function report_tindakan_therapis_search($tgl_awal,$periode, $trawat_tglapp_start ,$trawat_tglapp_end, $report_groupby, $start, $end, $cabang){
			...
			if ($report_groupby == '')
				...
				$sql_del = "delete from temp_jml_tindakan_therapis";
				$this->cabang->query($sql_del);
				
				$sql_rawat =   "insert into temp_jml_tindakan_therapis (tjtt_rawat)
								select * from
								(
			...
			$query_temp =  "select t.*, r.rawat_kode, r.rawat_nama
							from temp_jml_tindakan_therapis t
							left join perawatan r on r.rawat_id = t.tjtt_rawat
							order by rawat_kode";
	...
	function report_tindakan_searchtotal($tgl_awal,$periode, $trawat_tglapp_start ,$trawat_tglapp_end, $report_groupby, $cabang){
			...
			$query_temp =  "select 
								sum(t.tjtt_ref0) as tjtt_total_ref0,
								sum(t.tjtt_ref1) as tjtt_total_ref1,
								sum(t.tjtt_ref2) as tjtt_total_ref2,
								sum(t.tjtt_ref3) as tjtt_total_ref3,
								sum(t.tjtt_ref4) as tjtt_total_ref4,
								sum(t.tjtt_ref5) as tjtt_total_ref5,
								sum(t.tjtt_ref6) as tjtt_total_ref6,
								sum(t.tjtt_ref7) as tjtt_total_ref7,
								sum(t.tjtt_ref8) as tjtt_total_ref8,
								sum(t.tjtt_ref9) as tjtt_total_ref9,
								sum(t.tjtt_ref10) as tjtt_total_ref10,
								sum(t.tjtt_ref11) as tjtt_total_ref11,
								sum(t.tjtt_ref12) as tjtt_total_ref12,
								sum(t.tjtt_ref13) as tjtt_total_ref13,
								sum(t.tjtt_ref14) as tjtt_total_ref14,
								sum(t.tjtt_ref15) as tjtt_total_ref15,
								sum(t.tjtt_ref16) as tjtt_total_ref16,
								sum(t.tjtt_ref17) as tjtt_total_ref17,
								sum(t.tjtt_ref18) as tjtt_total_ref18,
								sum(t.tjtt_ref19) as tjtt_total_ref19,
								sum(t.tjtt_ref20) as tjtt_total_ref20,
								sum(t.tjtt_ref21) as tjtt_total_ref21,
								sum(t.tjtt_ref22) as tjtt_total_ref22
							from temp_jml_tindakan_therapis t";
	...
	function report_daftar_dokter($tgl_awal,$periode, $trawat_tglapp_start ,$trawat_tglapp_end ,$trawat_dokter, $report_groupby, $start, $end, $cabang, $print){
		$this->cabang = $this->load->database($cabang, TRUE);
