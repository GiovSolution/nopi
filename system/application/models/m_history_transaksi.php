...
class M_history_transaksi extends Model{
		...
		function history_transaksi_list($cust_id, $produk_id, $tanggal_start,$tanggal_end,$jenis,$query,$start,$end){
			...
			if ($produk_id == '')
				...
				$produk_filter = "AND produk.produk_id = '".$produk_id."'";
			...
			else if ($jenis == 'Produk')
			{
				$query = "SELECT customer.cust_id, customer.cust_no, customer.cust_nama,
								...
								detail_jual_produk.dproduk_harga as harga , detail_jual_produk.dproduk_diskon as diskon1, detail_jual_produk.dproduk_diskon2 as diskon2,

								(((`detail_jual_produk`.`dproduk_harga` * `detail_jual_produk`.`dproduk_jumlah`) - (((`detail_jual_produk`.`dproduk_harga` * `detail_jual_produk`.`dproduk_diskon`) * `detail_jual_produk`.`dproduk_jumlah`) / 100)) - ((((`detail_jual_produk`.`dproduk_harga` * `detail_jual_produk`.`dproduk_jumlah`) - (((`detail_jual_produk`.`dproduk_harga` * `detail_jual_produk`.`dproduk_diskon`) * `detail_jual_produk`.`dproduk_jumlah`) / 100)) * `detail_jual_produk`.`dproduk_diskon2`) / 100)) AS `subtotal`,
				...
				if($cust_id!=''){
					$query.=eregi("WHERE",$query)?" AND ":" WHERE ";
					$query.= " customer.cust_id = '".$cust_id."' ".$produk_filter."  and master_jual_produk.jproduk_stat_dok = 'Tertutup' and master_jual_produk.jproduk_tanggal BETWEEN '".$tanggal_start."' AND '".$tanggal_end."'";
				};
				...
				if($tanggal_start!='' && $tanggal_end!=''){
					...
					$query.=eregi("WHERE",$query)?" AND ":" WHERE ";
					$query.= " customer.cust_id = '".$cust_id."' ".$produk_filter."  and master_jual_produk.jproduk_stat_dok = 'Tertutup' and master_jual_produk.jproduk_tanggal='".$tanggal_start."'";
				}
