<?php
/* 	
	+ Module  		: Laporan Semua Produk Controller
...
//class of tindakan
class c_lap_jum_all_produk extends Controller {

	//constructor
	function c_lap_jum_all_produk(){
		parent::Controller();
		session_start();
	...
	function index(){
		...
		$this->load->view('main/v_lap_jum_all_produk');
	}
	...
	function get_action(){
		...
		switch($task){
			case "SEARCH":
				$this->report_tindakan_therapis_search();
				break;
			case "SEARCHTOTAL":
				$this->report_tindakan_searchtotal();
				break;
			case "PRINT":
				$this->report_jum_all_produk_print();
				break;
			case "EXCEL":
	...
	function report_tindakan_therapis_search(){
		//POST varibale here
		...
		$result = $this->m_lap_jum_all_produk->report_tindakan_therapis_search($tgl_awal,$periode, $trawat_tglapp_start ,$trawat_tglapp_end, $report_groupby, $start, $end, $cabang);
		echo $result;
	}
	...
	function report_tindakan_searchtotal(){
		...
		$result = $this->m_lap_jum_all_produk->report_tindakan_searchtotal($tgl_awal,$periode, $trawat_tglapp_start ,$trawat_tglapp_end, $report_groupby, $cabang);
		echo $result;
	}
	...
	function report_daftar_list_dokter()
	{
		...
		$start = (integer) (isset($_POST['start']) ? $_POST['start'] : $_GET['start']);
		$end = (integer) (isset($_POST['limit']) ? $_POST['limit'] : $_GET['limit']);
		$result = $this->m_lap_jum_all_produk->report_daftar_karyawan($tgl_awal,$periode, $trawat_tglapp_start ,$trawat_tglapp_end ,$trawat_dokter, $report_groupby, $start,$end, $cabang,0);
		echo $result;
	}
		...
		$start = (integer) (isset($_POST['start']) ? $_POST['start'] : $_GET['start']);
		$end = (integer) (isset($_POST['limit']) ? $_POST['limit'] : $_GET['limit']);
		$result = $this->m_lap_jum_all_produk->report_tindakan_search2($tgl_awal,$periode ,$trawat_tglapp_start ,$trawat_tglapp_end ,$trawat_dokter, $report_groupby, $start,$end);
		echo $result;
	}
	
	function report_jum_all_produk_print(){
  		//POST varibale here
		...
		$result = $this->m_lap_jum_all_produk->report_tindakan_print($tgl_awal,$periode, $trawat_tglapp_start ,$trawat_tglapp_end, $report_groupby, $cabang);
		$nbrows=$result->num_rows();
		
		$result2 = $this->m_lap_jum_all_produk->report_tindakan_print2($tgl_awal,$periode, $trawat_tglapp_start ,$trawat_tglapp_end, $report_groupby, $cabang);	
		...
		$end = 50;
		$list_dokter = '';
		$result_dokter = $this->m_lap_jum_all_produk->report_daftar_karyawan($tgl_awal,$periode, $trawat_tglapp_start ,$trawat_tglapp_end ,$trawat_dokter, $report_groupby, $start,$end, $cabang, 1);
		//$nbrows_dokter = $result_dokter->num_rows();
		//print_r ($result_dokter);
		$decode_result_dokter = json_decode($result_dokter, true);
		$numrows_dokter = count($decode_result_dokter);
		//print_r ($decode_result_dokter[0]['karyawan_nama']);
		...
		fwrite($file, "<body onload='window.print()'>
			<table summary='Tindakan List'>
			<caption>LAPORAN JUMLAH PENJUALAN PRODUK</caption>
			<thead>
				<tr>
					<th scope='col'>No</th>
					<th scope='col'>Kode</th>
					<th scope='col'>Perawatan</th>".$list_dokter."
				</tr>
			</thead>
	...
	function report_tindakan_export_excel(){
		...
		$query = $this->m_lap_jum_all_produk->report_tindakan_export_excel($tgl_awal,$periode, $trawat_tglapp_start ,$trawat_tglapp_end, $report_groupby, $cabang);
		$this->load->plugin('to_excel');
