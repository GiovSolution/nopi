...
<? //window.close();?>
<?php
function myheader($rproduk_tanggal ,$cust_no ,$cust_nama ,$cust_alamat, $cust_kota ,$rproduk_nobukti, $rproduk_jam, $karyawan_nama, $karyawan_no, $rproduk_keterangan, $karyawan_username){
?>
...
<table width="1200px" height="110px" border="0" cellspacing="0" cellpadding="0">
  ...
  <td height="50px" width="600px" colspan="2" align="left"><font size=3><b>NOTA  RETUR  PENJUALAN  PRODUK<br><font size=3>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?=$rproduk_nobukti;?></font></td>
  <tr>
...
?>
		...
		*/
		...
		$dcount = sizeof($detail_rproduk);
		foreach($detail_rproduk as $list => $row){
			$i+=1;
			if(($i%15)==1){
				myheader(
						$f_rproduk_tanggal,
	  ...
	  ?>
		...
		<td width="70px" align="right">&nbsp;
			<?=rupiah(($row->drproduk_harga));?></td>
  	    <td width="50px" align="right">&nbsp;
  	      <?=$row->drproduk_diskon;?>&nbsp; %</td>
		<td width="50px" align="right">Rp.</td>
  	    <td width="90px" align="right">&nbsp;
  	      <?=rupiah(($row->drproduk_jumlah)*($row->drproduk_harga)*((100-$drproduk_diskon)/100));?></td>
	    </tr>
  	  <?php 
			$subtotal+=(($row->drproduk_jumlah)*($row->drproduk_harga));
			
			if($i==$dcount){
				$total=($subtotal*((100-$drproduk_diskon)/100));
