...
/* 	
	...
	+ Filename 		: v_merk.php
 	+ Author  		: Isaac, Freddy
 	+ Created on 01/Apr/2012 22:40:00
...
/* declare function */		
var bank_DataStore;
var merk_ColumnModel;
var merkListEditorGrid;
var bank_createForm;
...
Ext.onReady(function(){
	...
	if(oGrid_event.record.data.bank_aktif!== null){bank_aktif_update = oGrid_event.record.data.bank_aktif;}
		...
		Ext.Ajax.request({  
			waitMsg: 'Please wait...',
			url: 'index.php?c=c_merk&m=get_action',
			params: {
				task: "UPDATE",
			...
			success: function(response){							
				...
				switch(result){
					...
					default:
						Ext.MessageBox.show({
							   title: 'Warning',
							   msg: 'Data Merk tidak bisa disimpan.',
	...
	function merk_create(){
		if(is_merk_form_valid()){
		
		var merk_id_create_pk=null;
		//var bank_kode_create=null;
		//var bank_nama_create=null;
		//var bank_norek_create=null;
		var merk_nama_create=null;
		//var bank_saldo_create=null;
		var merk_keterangan_create=null;
		var merk_aktif_create=null;

		merk_id_create_pk=get_pk_id();
		...
		if(merk_namaField.getValue()!== null){merk_nama_create = merk_namaField.getValue();}
		//if(bank_saldoField.getValue()!== null){bank_saldo_create = convertToNumber(bank_saldoField.getValue());}
		if(merk_keteranganField.getValue()!== null){merk_keterangan_create = merk_keteranganField.getValue();}
		if(merk_aktifField.getValue()!== null){merk_aktif_create = merk_aktifField.getValue();}
		...
		Ext.Ajax.request({  
				...
				params: {
					task: post2db,
					merk_id			: merk_id_create_pk,	
					//bank_kode		: bank_kode_create,	
					//bank_nama		: bank_nama_create,	
					//bank_norek	: bank_norek_create,	
					merk_nama		: merk_nama_create,	
					//bank_saldo	: bank_saldo_create,	
					merk_keterangan	: merk_keterangan_create,	
					merk_aktif		: merk_aktif_create
				}, 
				success: function(response){             
					var result=eval(response.responseText);
					switch(result){
						case 1:
							Ext.MessageBox.alert(post2db+' OK','Data Merk berhasil disimpan.');
							bank_DataStore.reload();
	...
	function get_pk_id(){
		if(post2db=='UPDATE')
			return merkListEditorGrid.getSelectionModel().getSelected().get('merk_id');
		else 
			return 0;
	}
	/* End of Function  */
	...
	function bank_reset_form(){
		...
		merk_namaField.reset();
		merk_namaField.setValue(null);
		bank_saldoField.reset();
		bank_saldoField.setValue(null);
		merk_keteranganField.reset();
		merk_keteranganField.setValue(null);
		merk_aktifField.reset();
		merk_aktifField.setValue('Aktif');
	}
 	/* End of Function */
  
	/* setValue to EDIT */
	function merk_set_form(){
		//bank_kodeField.setValue(merkListEditorGrid.getSelectionModel().getSelected().get('bank_kode'));
		//bank_namaField.setValue(merkListEditorGrid.getSelectionModel().getSelected().get('bank_nama'));
		//bank_norekField.setValue(merkListEditorGrid.getSelectionModel().getSelected().get('bank_norek'));
		merk_namaField.setValue(merkListEditorGrid.getSelectionModel().getSelected().get('merk_nama'));
		//bank_saldoField.setValue(CurrencyFormatted(merkListEditorGrid.getSelectionModel().getSelected().get('bank_saldo')));
		merk_keteranganField.setValue(merkListEditorGrid.getSelectionModel().getSelected().get('merk_keterangan'));
		merk_aktifField.setValue(merkListEditorGrid.getSelectionModel().getSelected().get('merk_aktif'));
	}
	/* End setValue to EDIT*/
  
	/* Function for Check if the form is valid */
	function is_merk_form_valid(){
		return (merk_namaField.isValid());
	}
  	/* End of Function */
	...
	function bank_delete(btn){
		if(btn=='yes'){
			var selections = merkListEditorGrid.selModel.getSelections();
			var prez = [];
			for(i = 0; i< merkListEditorGrid.selModel.getCount(); i++){
	...
	bank_DataStore = new Ext.data.Store({
		...
		reader: new Ext.data.JsonReader({
			...
			{name: 'merk_id', type: 'int', mapping: 'merek_id'},
			...
			{name: 'merk_keterangan', type: 'string', mapping: 'merek_keterangan'},
			{name: 'merk_aktif', type: 'string', mapping: 'merek_aktif'},
			{name: 'merk_creator', type: 'string', mapping: 'merek_creator'},
			{name: 'merk_date_create', type: 'date', dateFormat: 'Y-m-d H:i:s', mapping: 'merek_date_create'},
			{name: 'merk_update', type: 'string', mapping: 'merek_update'},
			{name: 'merk_date_update', type: 'date', dateFormat: 'Y-m-d H:i:s', mapping: 'merek_date_update'},
			{name: 'merk_revised', type: 'int', mapping: 'merek_revised'}
		]),
		sortInfo:{field: 'merk_id', direction: "ASC"}
	});
	/* End of Function */
	...
	cbo_bank_akunDataStore = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({
			url: 'index.php?c=c_merk&m=get_akun_list', 
			method: 'POST'
		}),
			reader: new Ext.data.JsonReader({
			root: 'results',
	...
	proxy: new Ext.data.HttpProxy({
			url: 'index.php?c=c_merk&m=get_mbank_list', 
			method: 'POST'
		}),
			reader: new Ext.data.JsonReader({
			root: 'results',
	...
	merk_ColumnModel = new Ext.grid.ColumnModel(
		[{
			header: '#',
			readOnly: true,
			dataIndex: 'bank_id',
		...
		*/
		{
			...
			width: 150,
			sortable: true
			<?php if(eregi('U',$this->m_security->get_access_group_by_kode('MENU_MERK'))){ ?>
			,
		...
		*/
		...
		},
		{
			...
			editor: new Ext.form.ComboBox({
				...
				store:new Ext.data.SimpleStore({
					fields:['merk_aktif_value', 'merk_aktif_display'],
					data: [['Aktif','Aktif'],['Tidak Aktif','Tidak Aktif']]
					}),
				mode: 'local',
               	displayField: 'merk_aktif_display',
               	valueField: 'merk_aktif_value',
               	lazyRender:true,
               	listClass: 'x-combo-list-small'
            })
			<?php } ?>
		...
		{
			header: 'Revised',
			dataIndex: 'merk_revised',
			width: 150,
			sortable: true,
			hidden:true
		}]
	);
	merk_ColumnModel.defaultSortable= true;
	/* End of Function */
    
	/* Declare DataStore and  show datagrid list */
	merkListEditorGrid =  new Ext.grid.EditorGridPanel({
		id: 'merkListEditorGrid',
		el: 'fp_bank',
		title: 'Merk',
		autoHeight: true,
		store: bank_DataStore, // DataStore
		cm: merk_ColumnModel, // Nama-nama Columns
		enableColLock:false,
		frame: true,
		...
		<?php if(eregi('U|R',$this->m_security->get_access_group_by_kode('MENU_BANKREK'))){ ?>
		{
			...
			iconCls:'icon-update',
			handler: merk_confirm_update   // Confirm before updating
		}, '-',
		<?php } ?>
	...
	function bank_editContextMenu(){
      merkListEditorGrid.startEditing(bank_SelectedRow,1);
  	}
	/* End of Function */
  	
	merkListEditorGrid.addListener('rowcontextmenu', onbank_ListEditGridContextMenu);
	bank_DataStore.load({params: {start: 0, limit: pageS}});	// load DataStore
	merkListEditorGrid.on('afteredit', bank_update); // inLine Editing Record
	...
	merk_namaField= new Ext.form.TextField({
		id: 'merk_namaField',
		fieldLabel: 'Nama Merk <span style="color: #ec0000">*</span>',
		maxLength: 250,
		allowBlank: false,
		anchor: '95%'
	});
	...
	merk_keteranganField= new Ext.form.TextArea({
		id: 'merk_keteranganField',
		fieldLabel: 'Keterangan',
		allowBlank: true,
		anchor: '95%'
	});
	/* Identify  bank_aktif Field */
	merk_aktifField= new Ext.form.ComboBox({
		id: 'merk_aktifField',
		fieldLabel: 'Status',
		store:new Ext.data.SimpleStore({
			fields:['merk_aktif_value', 'merk_aktif_display'],
	...
	bank_createForm = new Ext.FormPanel({
		...
		items: [{
			...
			items:[
			{
				...
				layout: 'form',
				border:false,
				items: [merk_namaField, merk_keteranganField, merk_aktifField] 
			}
			]
		}]
		,
